...
mod imp {
    ...
    impl TrayManager {
        ...
        fn load_tray_icon(is_light: bool) -> Icon {
            ...
            Icon::from_rgba(rgba_vec, width, height)
...
mod imp {
    ...
    impl TrayManager {
        pub fn new(_is_light: bool) -> Self {
            // TODO: 后续根据 macOS 菜单栏和 Linux StatusNotifier/AppIndicator 支持情况接入托盘。
            Self
        }

        pub fn update_theme(&mut self, _is_light: bool) {}

        pub fn update_item_text(&self, _visible: bool) {}
    }
}
